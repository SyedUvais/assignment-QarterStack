...
const TeamWork = ({
	...
	// console.log("syed project data", projects);
	...
	console.log('syed users array on team work', users);
	const [slowDownUser, setSlowDownUser] = useState('');
	const [absentUser, setAbsentUser] = useState('');
	const [idleUser, setIdleUser] = useState('');
	...
	useEffect(() => {
		...
		users.forEach((curUser, index) => {
			if (curUser.active_todo_count == 0 && curUser.user_id !== 34188691) {
				console.log('absent user, index', curUser.active_todo_count, index);
				setAbsentUser(prevVal => prevVal + curUser.name.split(' ')[0] + ', ');
			}

			if (
				moment().diff(moment(curUser.last_active_at), 'hours') >= 3 &&
				curUser.user_id !== 34188691
			) {
				console.log('slowdown user, index', curUser.name, index);
				setSlowDownUser(prevVal => prevVal + curUser.name.split(' ')[0] + ', ');
			}

			if (curUser.tasks_count == 0) {
				console.log('idle user, index', curUser.name, index);
				setIdleUser(prevVal => prevVal + curUser.name.split(' ')[0] + ', ');
			}
		});
	}, [data]);
		...
		<>
			...
			>
				<Container>
					...
					{showSlowdownComponent && (
						<div className='slowDownCmponent' style={{ marginTop: '20px', color: 'white' }}>
							{/* props.active_todo === 0 ? 'red' : (moment().diff(moment(props.last_active_at), 'hours') >= 3 ? '#EDFC45' : 'white' */}
							{console.log(
								'split array: slowdown, absent, idle',
								slowDownUser.split(', '),
								absentUser.split(', '),
								idleUser.split(', ')
							)}
							{slowDownUser && slowDownUser.split(' ').length - 1 ? (
								<p>
									{slowDownUser.split(' ').length - 1} Slowdowns: {slowDownUser}
								</p>
							) : (
								<p >{slowDownUser.split(' ').length - 1} Slowdowns: <span style={{color:'red'}}>No person in slowdown</span></p>
							)}
							{absentUser && absentUser.split(' ').length - 1 ? (
								<p>
									{absentUser.split(' ').length - 1} Absents: {absentUser}
								</p>
							) : (
								<p >{absentUser.split(' ').length - 1} Absents: <span style={{color:'red'}}>No person in Absents</span></p>
							)}
							{idleUser && idleUser.split(' ').length - 1 ? (
								<p>
									{idleUser.split(' ').length - 1} Idles: {idleUser}
								</p>
							) : (
								<p >{idleUser.split(' ').length - 1} Idles: <span style={{color:'red'}}>No person in Idles</span></p>
							)}
						</div>
					)}
					...
					{/* users segmenting */}
					<section>
						<h1 style={{textAlign:'center', color:'white', marginTop:'50px', marginBottom:'20px'}}>Team Members Segmenting</h1>
						<section>
							<div>
								<h4 style={{color: 'white' }}>Playground Segmenting: (Client Projects Team) </h4>
								...
								<thead>
										...
										<tbody>
											{users
												? users.map((user, key) => {
													if (user.project_ids.length && !user.project_ids.includes(23190856))
													{
														console.log("redwing project", user)
														return (
															<TableRow
																key={key}
																img={user.avatar}
							...
							<div style={{marginTop:'20px'}}>
								<h4 style={{color: 'white' }}>Playground Segmenting: (Redwing Team) </h4>
										...
										<tbody>
											{users
												? users.map((user, key) => {
													...
													{
														console.log("client project", user)
														return (
															<TableRow
																key={key}
																img={user.avatar}
						...
						<section style={{marginTop:'50px'}}>
							<div>
								<h4 style={{color: 'white' }}>Project Segmenting: Multitasking Table </h4>
							...
							<div style={{marginTop:'50px', color:'white'}}>
								<h4 style={{color: 'white' }}>Project Segmenting: List of projects with enrolled team members</h4>
											...
											? projects.map((project, key) => {
													return (
														<tr key={key}>
															<td>{project.name}</td>
															{users.map((user, key) => {
																if (user.project_ids.includes(project.project_id))
																{
																	return (
							...
							<div style={{marginTop:'50px', color:'white'}}>
								<h4 style={{color: 'white' }}>Project Segmenting: Idle Table</h4>
										...
										<tbody>
											{users
												? users.map((user, key) => {
													...
													{
														console.log("idle table", user.tasks_count)
														return (
															<TableRow
																key={key}
																img={user.avatar}
						...
						<section>
							<div style={{marginTop:'50px'}}>
								<h4 style={{color: 'white' }}>Performance Segmenting: (Table 1:- Users in descending manner based on green ticks) </h4>

								<table cellspacing='0' cellpadding='0'>
										<tbody>
											{users
												? [...users].sort((a, b) => (a['completed_todo'] < b['completed_todo'] ? 1 : -1)).map((user, key) => {
														return (
															<TableRow
																key={key}
																img={user.avatar}
							...
							<div style={{marginTop:'50px'}}>
								<h4 style={{color: 'white' }}>Performance Segmenting: (Table 2:- Users with 5+ green ticks) </h4>
							...
							<div style={{marginTop:'50px'}}>
								<h4 style={{color: 'white' }}>Performance Segmenting: (Table 3:- Users with 1 to 5 green ticks) </h4>
							...
							<div style={{marginTop:'50px'}}>
								<h4 style={{color: 'white' }}>Performance Segmenting: (Table 4:- Users with no green tick) </h4>
				...
				<Grid container spacing={2}>
					...
					<Grid item xs={8} sm={10} style={{ fontSize: '14px' }}>
						<a
							...
							style={{
								color:
									props.active_todo === 0 && props.user_id !== 34188691
										? 'red'
										: moment().diff(moment(props.last_active_at), 'hours') &&
										  props.user_id !== 34188691 >= 3
										? '#EDFC45'
										: 'white',
								paddingLeft: '2rem',
								fontSize: '14px'
